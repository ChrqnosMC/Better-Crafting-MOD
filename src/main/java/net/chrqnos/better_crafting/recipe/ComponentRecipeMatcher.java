...
public class ComponentRecipeMatcher {
    ...
    public class Matcher {
        ...
        private final IntList ingredientItemLookup = new it.unimi.dsi.fastutil.ints.IntArrayList();
        ...
        public Matcher(final Recipe<?> recipe) {
            ...
            for (int i = 0; i < this.ingredients.size(); ++i) {
                List<RecipeKey> requiredList = Arrays.stream(((Ingredient)this.ingredients.get(i)).getMatchingStacks()).map(ComponentRecipeMatcher::getKey).toList();
                for (int j = 0; j < this.totalRequiredItems; ++j) {
                    Iterator var6 = requiredList.iterator();
                    while(var6.hasNext()) {
                        RecipeKey required = (RecipeKey)var6.next();
        ...
        private int getMaximumCrafts() {
            int maxCraftabe = Integer.MAX_VALUE;
            for (Ingredient ingredient : this.ingredients) {
                int ingredientMax = 0;
                ...
                if (maxCraftabe > 0) {
                    maxCraftabe = Math.min(maxCraftabe, ingredientMax);
                }
            }
            return maxCraftabe;
        }
